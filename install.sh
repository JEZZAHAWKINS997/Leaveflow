...
print_summary() {
    ...
    echo ""
    echo -e "Your Supabase credentials:"
    echo -e "  • ${BLUE}API URL:${NC}          http://localhost:8000"
    echo -e "  • ${BLUE}Anon Key:${NC}         (see .env file)"
    echo -e "  • ${BLUE}Service Key:${NC}      (see .env file)"
    echo ""
    ...
    echo ""
    echo -e "To connect LeaveBoard app, use these environment variables:"
    echo -e "  VITE_SUPABASE_URL=http://localhost:8000"
    echo -e "  VITE_SUPABASE_ANON_KEY=(from .env file)"
    echo ""
}
